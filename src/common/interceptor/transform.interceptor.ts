...
export class TransformInterceptor<T> implements NestInterceptor<T, Response<T>> {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<Response<T>> {
    ...
    const createBy = this.reflector.get<boolean>("createBy", context.getHandler());
    if (createBy) {
    ...
    const updateBy = this.reflector.get<boolean>("updateBy", context.getHandler());
    if (updateBy) {
      request.body.updateBy = request["user"]?.userId;
      request.body.updateTime = Math.floor(Date.now() / 1000);
    }

    return next.handle().pipe(map((data) => ({ data, code: "00000", msg: "Success" })));
  }
}
